...
int DigraphTransitiveClosure (PtDigraph pdig)
{
    ...
    for(Vert = pdig->Head; Vert != NULL; Vert = Vert->PtNext) {
        ...
        for(i = 1; i <= pvlist[0]; i++)
            /*   A função InEdge já verifica se a aresta a inserir já existe.
             *   Em caso afirmativo, retorna REP_EDGE, se bem que neste caso é
             * irrelevante.
             *   Assim sendo, é ignorada essa verificação por uma questão de
             * eficiência. */
